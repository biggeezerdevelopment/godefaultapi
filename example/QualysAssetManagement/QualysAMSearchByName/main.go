...
func fetchAssets(client *godefaultapi.Client, host bool, name string, startFromId string, limitResults string) ([]map[string]interface{}, error) {
	count++
	...
	if response.ServiceResponse.HasMoreRecords == "true" {
		...
		nextAssets, err := fetchAssets(client, host, name, nextId, limitResults)
		if err != nil {
			return nil, err
		}
...
func main() {
	...
	name := flag.String("name", "", "Name to search for")
	...
	assets, err := fetchAssets(client, *host, *name, "0", *limitResults)
	if err != nil {
		log.Fatal(err)
	}
	...
	if *host {
		err = os.WriteFile("hostsbyname.json", file, 0644)
	} else {
		err = os.WriteFile("assetsbyname.json", file, 0644)
	}
	if err != nil {
		log.Fatal(err)
	}
	if *host {
		fmt.Printf("Total tagged host retrieved: %d\n", len(assets))
	} else {
		fmt.Printf("Total tagged asset retrieved: %d\n", len(assets))
	}

	fmt.Println("Tagged Assets written to assetsbyname.json")
}
